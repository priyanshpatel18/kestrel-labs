...
exports.parsePriceUpdateAccount = parsePriceUpdateAccount;
exports.readOracleSnapshot = readOracleSnapshot;
exports.priceAsFloat = priceAsFloat;
 ...
 */
function parsePriceUpdateAccount(data) {
    if (data.length < 8 + 32 + 1 + 92)
        return null;
    const discrim = data.subarray(0, 8);
    if (!discrim.equals(PRICE_UPDATE_V2_DISCRIM))
        return null;
...
async function readOracleSnapshot(params) {
    try {
        ...
        params.log?.warn({ feed: params.feed.toBase58(), err: String(err?.message || err) }, "oracle: read failed");
        return null;
    }
}
function priceAsFloat(snap) {
